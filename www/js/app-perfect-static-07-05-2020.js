...
$(document).on('page:init', '.page[data-name="dpr_sheetnew"]', function (page) {
  menuload();
  checkConnection();
...
$(document).on('page:init', '.page[data-name="dpr_sheet"]', function (page) {
  ...
  if(sess_designation=='SGL EIC'){
    ...
    var change_yr = change_dt_format[0];
    var change_mm = change_dt_format[1];
    var change_dd = change_dt_format[2];
    var changed_dpr_dt = change_dd+"-"+change_mm+"-"+change_yr;
    alert("changed_dpr_dt "+changed_dpr_dt);
  ...
  $.ajax({
    ...
    success:function(result){
      ...
      //var lcv_count = parse_res.lcv_cnts;
...
/*          if(g1 < g2){  
      ...
      for(var i=1;i<=dispanser_count;i++){
        ...
        for(var d_a=0;d_a<=23;d_a++){
          ...
          // }else{
          //   //alert("ELSE");
          //   disp_html+='<td class="text-uppercase disp_tr_'+i+' '+dis_a+' pl-0 disp_th_'+arr_ser[d_a]+'"><input type="number" name="para[DISP_'+i+'_A]['+addzero_a+'_00]" class="td_txt disp_'+i+'_a_'+d_a+' val_disp_1'+'_'+d_a+'" ></td>';
          // }
          disp_html+='<td class="text-uppercase disp_tr_'+i+' '+dis_a+' pl-0 disp_th_'+arr_ser[d_a]+'"><input type="number" name="para[DISP_'+i+'_A]['+addzero_a+'_00]" class="td_txt '+readonly_cls+' disp_'+i+'_a_'+d_a+' val_disp_1'+'_'+d_a+'" '+read_only+'></td>';e
